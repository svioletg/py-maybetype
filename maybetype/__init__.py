...
class Maybe[T]:
    ...
    def int(val: Any) -> 'Maybe[int]':  # noqa: ANN401
        """
        Attempts to convert `val` to an `int`, returning a `Maybe`-wrapped `int` if successful, or `Maybe(None)`
        on failure.
        """
        try:
            i: int = int(val)
    ...
    def unwrap(self,
        ...
        :param exc_args: Arguments to call `exc` with, if `exc` is a `Callable`. Otherwise, this argument is not used.
        """
        if self.val is None:
            if exc is None:
    ...
    def attr[V](self, name: str, typ: type[V] | None = None, *, err: bool = False) -> 'Maybe[V]':
        ...
        :param err: If `True`, `AttributeError` is raised instead of returning `Maybe(None)` if `name` does not exist.
        """
    ...
    def get[V](self,
        ...
        `__getitem__`. If it does not, or if the value does not exist (list index out of range, key does not exist on a
        dictionary, etc.), `Maybe(None)` is returned.

        :param typ: Specifies the generic type of the resulting `Maybe`. Note that the potential value returned by
            this method will not be coerced to the given type at runtime; this argument is only for typing purposes.
        :param err: By default, `IndexError` and `KeyError` are not raised when `__getitem__` is called on the wrapped
            value, and `Maybe(None)` is returned instead. Setting `err` to `True` allows these errors to be raised as
            they normally would. Note that if `__getitem__` did not exist on the wrapped value in the first placed (such
            as with `Maybe(None)`), no error is raised, and `Maybe(None)` is returned regardless.
